...
function updateSaveIndicator() {
    ...
    if (isSaving) {
        ...
        // Mostra "Salvo na nuvem" se não estiver sujo, ou nada se estiver limpo logo após load
...
export function setDataDirty(value: boolean) {
    ...
    if (value) {
        const el = document.getElementById('save-status');
        if (el) el.innerHTML = `<span style="color: var(--warning-color)">• Alterações pendentes</span>`;
